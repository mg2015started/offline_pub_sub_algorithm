...
void Printer::SaveDetails(string fileName, vector<double > &matchDetailList, int i)
{
	string text = "\nmatching " + Util::Int2String(i) + " pub\n";
	for (unsigned int i=0; i < matchDetailList.size(); ++i)
	{
		text += Util::Double2String(matchDetailList[i]);
		text += "\n";
	}
	Util::WriteData(fileName, text);
};
