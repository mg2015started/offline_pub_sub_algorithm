...
void Util::Check(generator &gen, string filename)
{
    ...
    ofstream filesStream;
    filesStream.open(filename);
    filesStream << Util::Average(matchTimeList) << "\t" << Util::Double2String(Util::Average(matchSubList));
}
...
void Util::disdetail(string i)
{
	vector<double> prevec;
	vector<double> searchvec;
	vector<double> traversevec;
	vector<double> tagvec;
	double time;
	ifstream fileStream;
	fileStream.open("./Detail/pretime0.txt");
	while(fileStream >> time)
		prevec.push_back(time);
	fileStream.close();
	fileStream.open("./Detail/searchtime0.txt");
	while(fileStream>>time)
		searchvec.push_back(time);
	fileStream.close();
	fileStream.open("./Detail/traversetime0.txt");
	while(fileStream>>time)
		traversevec.push_back(time);
	fileStream.close();
	fileStream.open("./Detail/tagtime0.txt");
	while(fileStream>>time)
		tagvec.push_back(time);
	fileStream.close();
	ofstream f1;
	f1.open(string("./Detail/dt/dt")+i+string(".txt"));
	f1 << Util::Double2String(Util::Average(prevec)) + "\t" << Util::Double2String(Util::Average(searchvec)) + "\t" +
	Util::Double2String(Util::Average(traversevec)) + "\t" + Util::Double2String(Util::Average(tagvec)) << "\n";
	f1.close();
	...
	traversevec.clear();
	tagvec.clear();
	fileStream.open("./Detail/pretime1.txt");
	while(fileStream >> time)
		prevec.push_back(time);
	fileStream.close();
	fileStream.open("./Detail/searchtime1.txt");
	while(fileStream>>time)
		searchvec.push_back(time);
	fileStream.close();
	fileStream.open("./Detail/traversetime1.txt");
	while(fileStream>>time)
		traversevec.push_back(time);
	fileStream.close();
	fileStream.open("./Detail/tagtime1.txt");
	while(fileStream>>time)
		tagvec.push_back(time);
	fileStream.close();
	f1.open(string("./Detail/dt/dt")+i+string(".txt"),ios::app);
	f1 << Util::Double2String(Util::Average(prevec)) + "\t" << Util::Double2String(Util::Average(searchvec)) + "\t" +
															   Util::Double2String(Util::Average(traversevec)) + "\t" + Util::Double2String(Util::Average(tagvec));
	f1.close();
